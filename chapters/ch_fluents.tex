...
So, the algorithm proceeds as follows. Initially, $E$ is set to a low number,
...
\begin{figure}
	\centering
	\begin{tikzpicture}[
			step node/.style={circle, draw, inner sep=0pt, minimum size=0.7cm, thick},
			node distance=0.7cm,
		]
		\matrix (steps) [matrix of nodes, nodes=step node, column sep=1.7cm] {
			1\& 2\& 3\& 4\\
		};
		%
		\node (label 1) [below=of steps-1-1] {train\\the agent};
		\node (label 2) [above=of steps-1-2] {train the\\ RB features extractor};
		\node (label 3) [below=of steps-1-3] {train the\\ restrained agent};
		\node (label 4) [above=of steps-1-4] {expert play};
		%
		\draw [pin line] (label 1) edge (steps-1-1)
		                 (label 2) edge (steps-1-2)
		                 (label 3) edge (steps-1-3)
		                 (label 4) edge (steps-1-4);
		\draw [flow] (steps-1-1) -- (steps-1-2);
		\draw [flow] (steps-1-2) -- (steps-1-3);
		\draw [flow] (steps-1-3) -- (steps-1-4);
		\draw [flow] (steps-1-4.east) -- +(0.5,0);
		\draw [flow, dashed, gray] (steps-1-4) -- ++(0,-2.3) -| (steps-1-2);
	\end{tikzpicture}
...
the Atari games and the training algorithm is Double DQN~\cite{bib:double-q},
described in Section~\ref{sec:deep-q-agents}. The agent model is the Deep
Q-Network defined in Section~\ref{sec:model-atari}.

The only rewards and observations the agent receives during this phase are
those generated by the environment. This training produces an agent that, with
some success, tries to maximize the environment rewards. As we've discussed in
the previous chapter, the agent's performances may be really low in presence
of partial observations. In some environments, the agent may not learn
anything at all (this is the case for Montezuma's Revenge game, for example).
This is fine, as we don't need specific capabilities to proceed to the next
phase.
...
(Section~\ref{sec:tempoal-goals}) or improve its performances on partial
observations (Section~\ref{sec:partial-obs}), we can apply the Restraining
Bolt. First, we need to select a set of fluents $\fluents$, useful in the
definition of some \ltl{}/\ldl{} temporal specification.
...
must be learnable with the complete model we've described here. One of the
assumptions, for example, is that the truth of each of these propositions
change, at least sometimes. This means that we cannot learn symbols that the
agent obtained from step 1 is not able to influence (consider for example a
symbol $\const{AgentAtEnd}$, where End is a position in a maze, that the agent
is unable to reach, yet).
...
Every symbol $\fluent \in \fluents$ has an associated region from which it is
valuated. For each of these regions, we train an encoder as shown in
Figure~\ref{fig:step2-encoder}.
\begin{figure}
	\centering
	\begin{tikzpicture}

		\node (agent) [block] {Agent};
		\node (env) [block, below=2cm of agent] {Environment};
		\node (play label) [above left=0.5cm of agent, tight] {play};
		\begin{pgfonlayer}{below}
			\node (play box) [fit=(agent) (env) (play label), box=orange] {}; 
		\end{pgfonlayer}

		\coordinate (agent out pt) at ($(agent.south)+(-0.25,0)$);
		\coordinate (agent in pt) at ($(agent.south)+(0.25,0)$);
		\draw [arrow] (agent out pt) --
			node [near end, left] {$a_{t-1}$}
			(agent out pt |- env.north);
		\draw [arrow] (env.north -| agent in pt) --
			node [near end, right] {$\bvec{o}_t$} 
			coordinate [midway] (observations link)
			(agent in pt);

		\node [block, right=3cm of observations link] (dataset)
			{Dataset\\collection};
		\draw (observations link -| play box.east)
			edge [arrow, -, dashed] (observations link) edge [arrow] (dataset);

		\node [block, right=2cm of dataset] (dbn) {DBN\\Encoder};
		\draw [arrow] (dataset) --
			node [above, midway] {$\set{\bv\group{i}}$}
			(dbn);

		\node (train label) [tight, right] at (play label.east -| dataset.west)
			{train};
		\begin{pgfonlayer}{below}
			\node (train box) [fit=(dataset) (dbn) (dataset|-env.south)
				(train label), box=blue!50!lightgray] {}; 
		\end{pgfonlayer}

	\end{tikzpicture}
	\caption{Training step 2 -- the encoder.}
	\label{fig:step2-encoder}
\end{figure}
On the left, there is the agent obtained from the previous phase that
continuously acts (plays) with the environment. The agent is not trained here,
its only purpose is to produce the stream of observations~$\bvec{o}_t$. These
observations are cropped to the region of this encoder and collected in a
dataset. This is then used as training set for the Deep Belief Network of this
encoder. For efficiency, we don't need to store every observation. Instead,
the dataset contains just the most recent $n$ samples, for a $n$ large enough.
Then, at each training step, we sample a random batch $\set{\bv\group{i}}$
from it.

The role of the agent is not to achieve high rewards. Instead, it should
explore the environment states as thoroughly as possible. Varied observations
are needed to train a good encoder, because every input should have a
meaningful encoding vector assigned. For this reason, it's always better to
prefer exploration policies, which usually include a good amount of
stochastic actions. We use a combination of classic and custom exploration
policies from Section~\ref{sec:exploration-policies}.

Once we've trained all the encoders, we can move on to the Boolean function.
As we know, they are learnt with a Genetic Algorithm which estimate the
all the valuation function at the same time. We take as best valuations, the
...
% TODO: In the following, ``agent'' refers to the
% Deep Reinforcement Learning agent defined in Section~\ref{sec:model-atari},
% while a ``restrained agent'' is the variant of
% Section~\ref{sec:model-atari-rb} which is meant to be combined with a
% Restraining Bolt. 

% TODO: Every trace, but which trace? Observation traces are generated by our actions.
