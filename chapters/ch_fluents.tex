\chapter{Learning to ground symbols trough RL}
...
\end{figure}
The two blocks at the bottom are the additions, and part of the solution.
...
different distribution of~$\x$~\footnote{ The function $\basisfun_c$ only
receives the values of the clique $c$.  In Equation~\eqref{eq:ugm-gradient},
we write $\x$ instead of $\x_c$, to ease the notation.}. The positive term
simply stands for the average value of that quantity, computed from the
samples in the training dataset; while in the right-most term, we take an
expectation on $\x$ according to the distribution induced by the model.
...
During this phase, the agent only receives observations and rewards generated
...
\end{figure}
On the left, the agent obtained from the previous phase continuously interacts
(plays) with the environment. The agent is not trained here, its only purpose
is to produce the stream of observations~$\bvec{o}_t$. These observations are
cropped to the region of each encoder and collected in a dataset. This is then
used as training set for the Deep Belief Network. For efficiency, we don't
need to store every observation. Instead, the dataset contains just the most
recent $n$ samples, for a $n$ large enough.  Then, at each training step, we
sample a random batch $\set{\bv\group{i}}$ from it.
...
If, with the fluents available, we've been able to write a temporal
...
Regions are a reasonable simplification; their fixed position may be not.
...
A final issue with temporal constraints is that, because of the consistency
