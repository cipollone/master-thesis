...
The first step is to train a RL agent on the standard environment.
Some of the options of the \verb|agent train| command are: a batch size of 32,
learning rate of $0,0001$, double Q-Network update every 10000 steps. To
follow the training progress, we run TensorBoard on the agent's log directory.
The results obtained are shown in Figure~\ref{fig:breakout-agent-train}.
\begin{figure}
	\centering
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot table
			{./data/run-17may_logs-tag-episode_metrics_episode_reward.txt};
		\end{axis}
	\end{tikzpicture}
...
\end{figure}
The plot is the cumulative reward achieved in each episode. At the end of the
training, the agent achieves an average reward above 70, which means that is
able to hit 70 bricks without ever losing the ball. This was an expected
result, as we want to replicate the results of previous studies at this stage.

The neural network needs time to train on the new observations reached (a
frame without bricks is really different to the Q-Network). However, we stop
this training at 4100 episodes, as this is not the main purpose of this
experiment. These performances are sufficient to reach states in which
\const{Empty} becomes true. So, we're ready to train the features extractor.
