...
\begin{description}[style=nextline]
	...
	\item[\ref{ch:nonmarkovrl}~--~\nameref{ch:nonmarkovrl}]
		% TODO: new build on recent solution methods and propose some original
		% view
		...
		% TODO: in this section we also illustrate our original view, which is two
		% sources of non-Markovian in the rewards, in particular the standard one
		% coming from RB and a secondary one coming incomplete partial
		% obaservability of the environment 
		% TODO: in addition, in 4.3 we propose an original model that allows us to
		% apply RB method in the context of Deep RL
	...
	\item[\ref{ch:fluents}~--~\nameref{ch:fluents}]
		...
		this model.
		% TODO: not we illustrate, but we propose.
		% TODO: we propose a training method that grounds fluents evaluation to
		% the features of the environments
		% TODO: non dire also
		% TODO: finally, we formulate an appropriate training algorithm
	\item[\ref{ch:atarieyes}~--~\nameref{ch:atarieyes}]
		% TODO: the software that we've developed which implement.
		% TODO: present tense
		...
		% TODO: then we illustrate some interesting implement
		% TODO: rimuovi will
