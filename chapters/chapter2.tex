...
\begin{example}
	...
	there is no need to ask the players how the game has carried out.
...
\infty$.  Since the environments we will experiment with are video games, each
play is an episode and the total number of steps in each episode is finite.
...
\end{equation}
which forces the first action to be~$a$. Since the agent can only observe
outcomes of single actions, this is usually a much more convenient form for
updating the estimate of the expected discounted return. Also, the optimal
policy can be simply expressed as:
\begin{equation}
...
algorithms, estimate $q_\policy$ of the policy $\policy$ currently under
improvement $\policy \to \policy^*$; off-policy algorithms, instead, act
...
select the optimal action. Generalizing in $q$, we call this the \emph{greedy
policy}, because it always selects the best action according to $q$:
\begin{equation}
	\policy_q(s) \coloneqq \argmax_{a \in A} q(s, a)
	\label{eq:pol-greedy}
\end{equation}
Unfortunately, while learning, we only have an estimate of the optimal
...
choose some actions randomly. The \emph{\eps-greedy policy} is defined:
\begin{equation}
...
suboptimal regions.  We can note that if $\epsilon = 1$,
...
can only try out all actions by following the random policy. It will receive
low rewards but it will observe all the possible outcomes. This is the purpose
of exploration. After a while, the agent can begin to trust about it's
predictions. So, it may choose the most promising actions to achieve higher
rewards. This is the exploitation phase. The exploitation--exploration
trade-off is a general problem in RL. Unfortunately, there's no general
solution because it's not easy to determine when the policy is ``good
enough''. Usually, we need some compromise between the two.

% Linear annealed. More later
...
also affect the regions previously visited. In fact, any method introduces
some techniques in order to generate a stable training.
...
games. At each step, the agent sends one of the 18 actions and receives as
result a frame of the game and a reward. The reward is the increment in the
player's score. This is really the same interface that a human player would
use. Figure~\ref{fig:atari-frames} shows frames from few games in this
collection. 
...
introduced in Neural Fitted Q~iteration~\cite{bib:nfq}, DQN addressed some
causes of training instability. They also demonstrated that exactly the
same agent can be trained in many Atari games and achieve human-level
performances in many of those~\cite{bib:atari-deepq-nature}. These promising
results sparked a lively interest in Deep RL, recently.
...
To train the network, we ensure that, for each input $(s, a)$, the model
returns the expected value of the target~$y_{s,a}$. For this network, the
targets are the optimal state-action values to estimate. To do so, we select
the parameters that minimize the squared difference between the estimates and
the targets:
\begin{equation}
	\text{loss}(\params) \coloneqq \bigl(Q(s, a; \params) - y_{s,a} \bigr)^2
\end{equation}
where $y_{s,a}$ is the target value $q^*(s, a)$. This loss contains some
random variables. So, we minimize it through some stochastic optimization
algorithm. In Stochastic Gradient Descent (SGD), at each step $t$,
we observe an input ${(s_t, a_t)}$ and the associated target $y_t$, and we
take a small step toward the negative gradient of the loss:
\begin{equation}
...
unknown, because they depend recursively from the same optimal state-function
that we're trying to learn (see equation~\eqref{eq:q-bellman}). In classic RL,
this is not a problem: the 1-step approximation,
\begin{equation}
	r_{t+1} + \discount \max_{a \in A} \est{q}(s_{t+1}, a)
	\label{eq:1step-targets}
\end{equation}
or the n-step approximation, are a valid targets $y_t$ for the
function~$\est{q}$. By updating toward these values on the whole input space,
convergence is guaranteed. 

With neural networks, instead, by updating the parameters, we also modify the
...
the network being trained, $Q(s, a; \params)$, but they are computed from a
second net, $Q(s, a; \params')$. Every $C$ iterations, the target net is
updated to match the trained net, with the assignment: $\params' \gets
\params$. This keeps the targets constant for $C$ steps and helps to stabilize
the training.

Second, the network is not trained from the last sample, but from the recent
experience. At each step, the agent acts according to some exploration policy,
$a_t \sim \policy_e$. Each transition, of the form $\langle s_t, a_t, r_{t+1},
s_{t+1} \rangle$, is recorded in a buffer of size $n_r$, called ``experience
replay''. Then, at each training step, we sample a number of $n_b$ transitions,
thus creating a batch, and we perform an update $\params_{i+1} = \params_i -
\alpha\, g_i$ on the cumulative gradient $g_i$ of the whole batch.

% TODO: accorgimenti per atari games
% TODO: network
% TODO: policies?
