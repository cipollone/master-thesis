...
such as $\N$, and $\prec$ is a complete ordering relation on $T$, like~$<$. 
Finally, for a logic that defines a set $\fluents$ of atomic propositions, the
valuation function $V: T \times \fluents \to \{\true, \false\}$ assigns a
truth value to each proposition, in every instant of time. The atomic
propositions $\fluents$ are also called ``fluents'' (or simply ``symbols'', in
this thesis).  An equivalent and compact way of defining such structures is
with \emph{traces}. A trace $\trace$ is a sequence of propositional
interpretations of the fluents~$\fluents$. Each interpretation in the
sequence, $\trace(i) \in 2^\fluents$, is the set of true symbols at time~$i$.
$\trace(i, j)$ represents the trace between instants $i$ and~$j$.
...
naturally. However, the formula $\always\eventually \formula$ does not require
that $\formula$ becomes true an infinite number of times (in LTL, this formula
represents the ``response'' property); instead, it is satisfied exactly by
those traces in which $\formula$ is true at the last instant. It assumes a
completely different meaning.  Furthermore $\always\eventually \formula$ and
$\eventually\always \formula$ are both equivalent to $\eventually
(\const{Last} \land \formula)$, something that doesn't happen in standard LTL.
($\const{Last}$ is an abbreviation for $\lnot \next \true$ and it evaluates to
true at last instant only).  From this example, it should be clear that the
expressive power of the language has changed and LTL interpreted over finite
traces should be regarded as a different logic, that we will denote with
