\chapter{Reinforcement Learning for non-Markovian rewards}
...
\begin{example}
	...
	Boolean variable, \const{lightOn}, which reflects the status of the light.
...
\begin{definition}
	...
	(NMDP)\nomenclature{NMDP}{Non-Markovian Decision Process} is a tuple
...
Factored POMDPs can clearly be formulated as NMDPs, but we can be even more
...
\begin{definition}
	...
	(RDP)\nomenclature{RDP}{Regular Decision Process} to be a factored NMDP
...
doesn't precise the formalism in which these functions are defined, but
only the general properties they need to respect: it must be possible to
generate states and rewards from a finite state transition transducer. Since
\ldl{} exactly capture the class of regular properties on the trace $S^+$,
this definition and the original in~\cite{bib:rdp} are completely equivalent}.


\begin{comment}

% Factored hidden dynamics
The first restriction is that we'll only consider POMDPs whose hidden dynamics
can be modelled by a factored Boolean representation. More precisely, we
assume that the environment can be defined as follows.
\begin{definition}
	We define a POMDP $\modelsym_p \coloneqq \langle \setSym{P}, S, A, T, R,
	\obsS, O\rangle$, where:
	\begin{itemize}
		\item $\setSym{P}$ is a finite set of atomic propositions.
		\item $S \coloneqq \obsS \times \setSym{I}_\setSym{P}$ is a state space
			that is composed by an observable part, $\obsS$, and a hidden
			part,~$\setSym{I}_\setSym{P}$. Furthermore, the unobservable component
			is $\setSym{I}_\setSym{P} \coloneqq 2^\setSym{P}$, the set of Boolean
			configurations for~$\setSym{P}$.
		\item $A$ is the space of discrete actions, as usual.
		\item $T: S \times A \times S$ is the transition function.
		\item $R: \setSym{I}_\setSym{P} \to \R$ is the reward function, whose
			domain is restricted to the current configuration of~$\setSym{P}$.
		\item $O: S \to \obsS$ is the observation function that just extracts the
			observable component.
	\end{itemize}
	\label{def:pomdp-factored}
\end{definition}

The class of partially observable environments that can be modelled as a POMDP
of this definition have a relatively simple hidden dynamic, that can be
represented by the state of the Boolean variables~$\setSym{P}$. This set of
propositions completely explains the rewards generated and, together with
$\obsS$, also allow to predict future observations. As an example to explain
this concept, we can consider the Atari games. In many of those, it is
possible to define some (possibly unobservable) propositions, like
$\const{EnemyDead}$ and $\const{DoorOpen}$, that are sufficient to produce the
environment rewards and, together with the frame of the game, can be used to
predict the next observation.

The solution we propose proceeds as follows. First, we observe that any POMDP
can be expressed as a non-Markovian Decision Process over the space of
observations. Then, we exploit the decomposition above to reconstruct the
hidden configuration $\setSym{I}_\setSym{P}$ from the history of observations.
Assuming regular properties on this hidden dynamics, we'll be able to use
\ldl{} formulae to reconstruct $\setSym{I}_\setSym{P}$, and as a consequence
also the rewards. This will leave us with a model almost equivalent to a MDP
with \ldl{} restraining specifications, which we already know how to solve.
This procedure mostly follows the ideas presented in~\cite{bib:rdp}.

Any POMDP~$\modelsym_p$ can be formulated as a NMDP~$\modelsym_n \coloneqq
\langle \obsS, A, T_\obsS, R_\obsS \rangle$, where rewards and transitions are
defined over the history of observations. This is a general result for POMDPs.
The advantage of this model is that it only considers the visible quantities.
Still, computing transitions and rewards from $\obsS^+$ is unfeasible.
Instead, we want to restrict this dependency on the past as much as possible.

For this purpose, we introduce a new set~$\fluents$ of atomic propositions.
The set of fluents, $\fluents$, is composed of propositions that can be
decided from our observation space, i.e. we assume $\fluents$ to be such that
there exists an $f: \obsS \to 2^\fluents$, in all the following definitions.
We've previously called $f$ a ``valuation function'' or ``features
extractor''.  We now use the variables~$\fluents$ to represent the
``essential'' information about the observations. More precisely, we'll
formulate the following NMRDP:
\begin{definition}
	We define a class of NMDPs, $\modelsym_f \coloneqq \langle \fluents, S, A,
	T, R \rangle$, with the following structure:
	\begin{itemize}
		\item $\fluents$ is a finite set of atomic propositions.
		\item $S \coloneqq \obsS \times \setSym{I}_\fluents$ is the state space
			composed by a set $\obsS$ and $\setSym{I}_\fluents \coloneqq
			2^\fluents$, the set of configurations for the fluents~$\fluents$.
		\item $T: \obsS \times \setSym{I}_\fluents^+ \times S\to \R$ is the
			transition function, where the dependency on the past history is
			restricted just to $\setSym{I}_\fluents$, the trace of valuations
			produced for~$\fluents$.
		\item $R: \setSym{I}_\fluents^+ \to \R$ is the reward function.
	\end{itemize}
	\label{def:nmdp-fluents}
\end{definition}
While every POMDP can be expressed as the general NMDP of
Definition~\ref{def:nmdp-general}, not all of them can be formulated as the
class of NMDPs defined here. However, we argue that many factored POMDP of
Definition~\ref{def:pomdp-factored} can be formulated this way. Still,
without further assumptions on the nature of the observations $\obsS$, we
can't guarantee whether it is possible to extract a sufficient set of
fluents~$\fluents$ for this translation (sufficient in the sense of
Definition~\ref{def:nmdp-fluents}). 

We've now reduced the relevant informations about the history of observations,
from an intractable $\obsS^+$, to $\setSym{I}_\fluents^+$, which is a trace of
Boolean valuations.  The next step is to assume some regularities in the
functions $T$ and $R$, to express a large group of such NMDPs in a even
simpler formalism.

The model that we want to introduce is a small variant of a \emph{Regular
Decision Process} (RDP)\nomenclature{RDP}{Regular Decision Process}. The
reader can refer to~\cite{bib:rdp} for a complete definition of RDPs.
Essentially, they are particular NMDPs with a completely factored state space
$S = \setSym{I}_\fluents$, whose transition and reward functions are defined
through \ldl{} formulae over the symbols~$\fluents$. The model we define here
is the analogue of a RDP, but with a composite state space.

We informally define a class of NMDPs (composite RDPs) defined as a tuples
$\modelsym_r \coloneqq \langle \fluents, S, A, T, R \rangle$ where:
\begin{itemize}
	\item $\fluents$ is a finite set of atomic propositions.
	\item $S: \obsS \times \setSym{I}_\fluents$ is the composite state space.
	\item $T$ is the transition function, specified as a finite set of tuples,
		$(\formula_i, a, T_i)$. Each $\formula_i$ is an \ldl{} formula over the
		set of atoms $\fluents$ that indicates for which traces this tuple is
		active. When the $i$-th tuple is active, and action $a$ is performed, the
		probability of the next state $s \in S$, given the current observation $o
		\in \obsS$, is returned by the function $T_i: \obsS \times S \to \R$.
	\item $R$ is the reward function, defined as a finite set of pairs
		$(\formula_j, r_j)$, which associate the reward $r_j$ when $\formula_j$
		is satisfied by the trace produced for~$\fluents$.
\end{itemize}
This is just an informal definition because the formulae must obey additional
constraints, in order to guarantee that each trace activates a single formula
in the set~$T$ (refer to the complete definition for RDPs in \cite{bib:rdp}).

Until this point, we've just defined some models with more and more
restrictions. We'll now link the class of POMDPs we're interested in, to
composite RDPs. Since, RDPs can be solved through MDPs over a compact state
space, this will be considered as a solution.

Inspired by Lemma 3 of~\cite{bib:rdp}, we state the following:
\begin{proposition}
	Let $\modelsym_p \coloneqq \langle \setSym{P}, S, A, T, R, \obsS, O\rangle$
	be a POMDP of Definition~\ref{def:pomdp-factored}. If there exists a set of
	fluents~$\fluents$ such that in all reachable states, the value of every $p
	\in \setSym{P}$ can be computed through a function $f_p: \obsS \times
	\setSym{I}_\fluents^+$, with a regular dependency on
	$\setSym{I}_\fluents^+$, then, there exists a (composite) RDP $\modelsym_r
	\coloneqq \langle \fluents, \obsS \times \setSym{I}_\fluents, A, T', R'
	\rangle$ that generates the same rewards as~$\modelsym_p$.
\end{proposition}
This is important, because RDPs can be learnt by transforming them to
equivalent MDPs. As shown in~\cite{bib:rdp}, an RDP with a fully
factored state space, $S = 2^\fluents$, can be transformed to an MDP with a
state space $S \times Q_1 \times \dots \times Q_n$. Also, each component $Q_i$
is the set of states associated to an automaton $\automa_i$ generated from the
\ldl{} formula $\formula_i$ that appears in the RDP's transition and reward
functions. Similarly, we can show that a composite RDP can be transformed to
an MDP with the following state space $\obsS \times S \times Q_1 \times \dots
\times Q_n$.

There is an evident similarity between each of these two MDP state spaces and
those appearing in Definition~\ref{def:ldlf-eq-mdp} at
page~\pageref{def:ldlf-eq-mdp} (NMRDPs with \ldl{} rewards) and those at
page~\pageref{page:rb-extended-mdp} (MDPs with \ldl{} restraining
specifications), respectively. This parallelism is due to the fact that a
(possibly composite) RDP is an equivalent formalization for the behaviours
defined by these two models.

To conclude, a class of POMDPs $\modelsym_p \coloneqq \langle \setSym{P}, S,
A, T, R, \obsS, O\rangle$ can be learnt by performing RL over an MDP
$\modelsym \coloneqq \langle \obsS, A, T', R' \rangle$ with appropriate \ldl{}
restraining specifications from a set of fluents~$\fluents$. Providing a
complete proof for this result is behind the scope of this thesis. The purpose
of this section is just to suggest how to leverage the results
in~\cite{bib:rdp} in presence of a state space which is not completely
factored (which we called ``composite''). In the small remainder of this
section, we discuss how we could address partially observable environments,
from a practical perspective. 

Since the POMDP hidden variables, $\setSym{P}$, are unknown, we can't easily
define an appropriate set of fluents and the correct \ldl{} temporal
specifications to reconstruct them. However, suppose our goal is to
\emph{imitate} the rewards generated by the environment. Let $\fluents$ be a
set of fluents which can be computed from the available observations $o \in
\obsS$. If we can define an MDP with \ldl{} specifications, written from the
atoms in~$\fluents$, that generates the same rewards as the environment, then,
by applying RL on the equivalent MDP, we can learn the original POMDP. The
resulting composite state space $\obsS \times Q_1 \times \dots \times Q_m$ of
the Restraining Bolt is a complete and exhaustive set of observations, in the
sense that, the additional observation $\vec{q}$ restores the Markov property
for rewards. We don't even need to supply the additional rewards generated by
the RB. Figure~\ref{fig:rb-partial-obs} shows this arrangement.
\begin{figure}
	\centering
	\begin{tikzpicture}[
			every node/.append style={font=\small},
...
\begin{example}
	...
	through a switch, but now it can only observe images of the room. Suppose the
