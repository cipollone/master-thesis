...
\begin{definition}
	...
	\begin{itemize}
		...
		\item $T$ and $R$ are the transition and reward functions, defined over the
...
of this definition have a relatively simple hidden dynamic, that can
be represented as an assignment for a set of propositions. Usually, when the
decomposition above is not possible or not feasible, it means that either the
observable component is way too limited or that there is some continuous
hidden dynamics that cannot be abstracted this way. Still, many environments
can be modelled with such POMDP, because there exist a small set of hidden
conditions in the environment that complete the explanation for future
observations.  Consider, for instance, the Atari games. In many cases, there
exists a small set of discrete simulator variables, representing the current
hidden state of the game, which influences the result of the agent's actions.

The solution proposed proceeds as follows. First, we observe that any POMDP
...
This will leave us with a NMRDP with \ldl{} rewards, which we already know how
to solve. This is a similar procedure to that presented in~\cite{bib:rdp}.

A non-Markovian Decision Process is identical to a NMRDP except that also the
transition function can depend on the entire history of configurations.
\begin{definition}
...
The set of fluents~$\fluents$ is composed of propositions that can be decided
from our observation space, i.e. we assume $\fluents$ to be such that there
exists an $f: \obsS \to 2^\fluents$, in all the following definitions. We've
previously called $f$ ``valuation function'' or ``features extractor''. We now
use the variables~$\fluents$ to represent the ``essential'' information about
the observations. More precisely, we'll formulate the following NMRDP:
\begin{definition}
	...
	\begin{itemize}
		...
		\item $S \coloneqq \obsS \times \setSym{I}_\fluents$ is the state space
			...
			2^\fluents$, the set of all configurations for the fluents~$\fluents$.
		\item $T: \obsS \times \setSym{I}_\fluents^+ \to \R$ is a transition
			function where the dependency on the past history is restricted just to
			the trace of valuations produced for~$\fluents$.
		\item $R: \obsS \times \setSym{I}_\fluents^+ \to \R$ is the reward
			function, which follows a similar restriction.
	\end{itemize}
	\label{def:nmdp-fluents}
\end{definition}
Some, but not all POMDPs, can be expressed as the NMDPs of this definition. In
fact, it is not always possible to find a sufficient set of fluents~$\fluents$
(sufficient in the sense of the definition). We argue that many factored POMDP
of Definition~\ref{def:pomdp-factored} can be formulated this way.

We've reduced the relevant informations about history from an intractable
$\obsS^+$ to $\setSym{I}_\fluents^+$ which is a trace of Boolean valuations.
The next step is to exploit regularities in the functions $T$ and $R$ to
express this NMDP in a even simpler formalism.
...
\begin{itemize}
	...
	\item $T$ is the transition function, specified as a finite set of tuples,
		...
		probability of the next state $s \in S$, given the current $o \in \obsS$,
		is returned by the function $T_i: \obsS \times S \to \R$.
...
in the set~$T$. RDPs are defined in~\cite{bib:rdp}.
...
composite RDPs. Since, RDPs can be solved as MDPs over a compact state space,
this will be considered as a solution.
...
\begin{proposition}
	...
	\coloneqq \langle \fluents, S', A', T', R' \rangle$ that generates the same
	rewards as~$\modelsym_p$.
\end{proposition}
...
\ldl{} formula $\formula_i$ that appear in the RDP transition and reward
functions.

In a similar way, we can show that a composite RDP can be transformed to an
MDP with a state space $\obsS \times Q_1 \times \dots \times Q_n$.  In fact,
there are many analogies both between RDPs and NMRDPs with \ldl{} rewards, and
between composite RDPs and MDPs with restraining specifications. However, the
purpose of this section is not to provide complete proofs, but only to suggest
how to leverage the results in~\cite{bib:rdp} in presence of a composite state
space. 

This discussion served to say that it is indeed possible to lean a restricted
group of POMDPs by performing RL on an equivalent MDP. We said what is
necessary, but not how to do it in a practical way. We conclude this section
by suggesting how we might address partially observable environments. Clearly,
we can't provide guarantees here, as much depends on the nature of the
environments dynamics and observations.
