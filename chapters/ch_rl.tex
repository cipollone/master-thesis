...
\begin{example}
	...
	\begin{figure}
			...
			\graph [
				...
				0 -> [self loop, "$\lnot\const{LightOn}$"] 0;
				1 -> [self loop] 1;
				2 -> [self loop] 2;
				3 -> [self loop, "$\true$"] 3;
			}; 
...
in~$\params$. Given a MDP state~$s \in S$, this computes the state-action
...
networks, with the same architecture but different parameters~$\params_1,
\dots, \params_{\abs{Q}}$. At each step, given an input $(o, q)$ the agent may
use the network $\params_q$ to predict the actions values for the input~$o$.
...
separate parameters~$\params_1, \dots, \params_{\abs{Q}}$. For a
