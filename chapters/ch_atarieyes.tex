\chapter{Tool for learning symbols through RL: the AtariEyes package}
...
\mintinline{py3}|import atarieyes|. For development, it may be also useful to
...
All commands are divided in two groups. Those starting with \texttt{agent}
...
inside our models. Now, calling \mintinline{py3}|rescaling(x)| is
...
The last definitions of this module are the three exploration policies of
...
What is important is that this class defines a function \verb|compute_fitness|
...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		% Input
		\node [image] (frame)
			{\includegraphics[height=1.8cm]{./imgs/br0.png}};
...
of each encoder. Also, \verb|--train| selects which of these inner models
should be trained, when we call the \texttt{Fluents}' \verb|compute_all|
function.  Depending on which model we're working on, both the computation
graph and the metrics saved greatly change.
